...
class Dataset40200Seeder extends Seeder
{
    /**
    ...
    public function run(): void
    {
        ...
        DB::table('dataset40200')->truncate();
        
        $csvFile = database_path('../data/40200.csv');
        ...
        if ($handle !== FALSE) {
            ...
            while (($data = fgetcsv($handle, 1000, ';')) !== FALSE) {
                ...
                if (count($data) >= 15) {
                    $batch[] = [
                        ...
                        'dipasang_di_no_reg_sista' => $this->cleanData($data[7]),
                        'status_permintaan' => $this->cleanData($data[8]),
                        'status_penerimaan' => $this->cleanData($data[9]),
                        'status_pengiriman' => $this->cleanData($data[10]),
                        'inbox_dokumen' => $this->cleanData($data[11]),
                        'jabatan' => $this->cleanData($data[12]),
                        'site' => $this->cleanData($data[13]),
                        'assignment_status' => $this->cleanData($data[14]),
                        'created_at' => now(),
                        'updated_at' => now()
                    ];
                    ...
                    if (count($batch) >= $batchSize) {
                        DB::table('dataset40200')->insert($batch);
                        $this->command->info("Inserted batch: " . $rowCount . " rows processed");
                        $batch = [];
                    }
                } else {
                    $this->command->warn("Row {$rowCount} tidak memiliki 15 kolom, dilewati. Kolom yang ada: " . count($data));
                }
            }
            ...
            // Tampilkan statistik
            $totalRecords = DB::table('dataset40200')->count();
